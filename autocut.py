...
def benchmark(detailed):
    ...
    try:
        ...
        click.echo("\n💻 System Capabilities:")
        click.echo(f"   Hardware Acceleration: {'✅' if system_info.has_hardware_acceleration else '❌'}")
        click.echo(f"   Supported Encoders: {', '.join(system_info.available_encoders)}")
...
@click.option('--quick', '-q', is_flag=True,
              help='Quick demo with limited files')
@click.option('--pattern', '-p',
              type=click.Choice(['energetic', 'balanced', 'dramatic', 'buildup']),
              default='balanced', help='Editing pattern to demonstrate')
def demo(quick, pattern):
    ...
    try:
        ...
        if result.success:
            ...
            click.echo("❌ Demo failed - check test_media directory has video/audio files")
            sys.exit(1)
