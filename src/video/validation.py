...
SUPPORTED_VIDEO_FORMATS = {
    ...
    '.3gp', '.3g2', '.mts', '.m2ts', '.ts', '.vob', '.divx', '.xvid',
    '.asf', '.rm', '.rmvb', '.f4v', '.swf'
}
...
class VideoValidator:
    ...
    def validate_iphone_compatibility(self, file_path: str, 
    ...
    def validate_input_files(self, video_files: List[str], 
                           audio_file: str) -> ValidationResult:
        """
        ...
        if not video_files:
            ...
            for i, video_file in enumerate(video_files):
                ...
                if not video_result.is_valid:
                    failed_videos.append((i + 1, video_file, video_result.issues[0].message))
                    result.add_error(
    ...
    def _get_codec_info(self, file_path: str) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            '-show_entries', 'stream=codec_name,profile,level,pix_fmt,width,height,r_frame_rate,duration',
        ...
        try:
            ...
            raise VideoProcessingError(f"FFprobe output parsing failed: {e}")
        except subprocess.TimeoutExpired:
            raise VideoProcessingError("FFprobe command timed out")
        except Exception as e:
            raise VideoProcessingError(f"Codec analysis failed: {e}")
    
    def _check_codec_compatibility(self, codec_info: Dict[str, Any], 
    ...
    def _check_resolution_limits(self, codec_info: Dict[str, Any], 
                               result: ValidationResult) -> None:
    ...
    def _check_frame_rate_limits(self, codec_info: Dict[str, Any], 
                               result: ValidationResult) -> None:
    ...
    def _check_container_compatibility(self, codec_info: Dict[str, Any], 
                                     result: ValidationResult) -> None:
    ...
    def _check_moviepy_compatibility(self, file_path: str, 
                                   result: ValidationResult) -> None:
