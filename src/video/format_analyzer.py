...
class VideoFormatAnalyzer:
    ...
    def __init__(self) -> None:
        self.format_cache: Dict[str, Any] = {}

    def analyze_video_format(self, video_clip: VideoFileClip) -> Dict[str, Any]:
    ...
    def determine_optimal_canvas(self, video_clips: List[Any]) -> Dict[str, Any]:
        ...
        dominant_fps_category = max(fps_counts, key=lambda k: fps_counts[k])
    ...
    def detect_format_compatibility_issues(
        ...
        resolutions = {(fmt["width"], fmt["height"]) for fmt in formats}
        ...
        fps_values = {fmt["fps"] for fmt in formats}
        if len(fps_values) > 1:
            issues.append(
                {
        ...
        aspect_ratios = {round(fmt["aspect_ratio"], 3) for fmt in formats}
