...
class AdaptiveWorkerMonitor:
    ...
    def set_scale_callbacks(self, scale_down_cb: Callable[[int], None], 
    ...
    def start_monitoring(self):
        ...
        print(f"   🔍 Started adaptive monitoring (thresholds: {self.warning_threshold}%/{self.critical_threshold}%)")
        ...
        try:
            ...
            if memory_percent >= self.critical_threshold:
                if self.current_workers > 1:
                    ...
                    self._scale_down(new_workers, f"CRITICAL memory usage: {memory_percent:.1f}%")
            ...
            # Warning threshold - gradual scaling
                ...
                if (self.consecutive_warnings >= 2 and 
                    self.current_workers > 1 and
                    current_time - self.last_scale_down_time > self.min_time_between_adjustments):
                    
                    new_workers = max(1, self.current_workers - 1)
                    self._scale_down(new_workers, f"HIGH memory usage: {memory_percent:.1f}% (warning #{self.consecutive_warnings})")
            ...
            # Recovery threshold - scale back up gradually
                ...
                if (self.current_workers < self.initial_workers and
                    current_time - self.last_scale_down_time > self.min_time_between_adjustments * 2):
                    
                    new_workers = min(self.initial_workers, self.current_workers + 1)
                    self._scale_up(new_workers, f"Memory recovered: {memory_percent:.1f}%")
...
class ThreadSafeWorkerController:
    ...
    def scale_workers(self, new_count: int):
        ...
        print(f"   📝 Worker scale request: {new_count} (ThreadPoolExecutor limitation)")
