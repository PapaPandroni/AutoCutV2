...
def detect_musical_start(y: np.ndarray, sr: int, tempo: float, beats: np.ndarray) -> Tuple[float, float]:
    ...
    for i, energy in enumerate(rms_energy):
        if energy >= energy_threshold:
            ...
            end_idx = min(i + int(sustained_duration * sr / hop_length), len(rms_energy))
...
def detect_intro_duration(y: np.ndarray, sr: int, tempo: float, 
                         energy_threshold: float = 0.3,
                         min_intro: float = 0.5,
                         max_intro: float = 8.0) -> float:
    ...
    rms_norm = (rms_energy - np.min(rms_energy)) / (np.max(rms_energy) - np.min(rms_energy) + 1e-8)
    chroma_norm = (chroma_energy - np.min(chroma_energy)) / (np.max(chroma_energy) - np.min(chroma_energy) + 1e-8)
...
def create_beat_hierarchy(beats: np.ndarray, tempo: float, sr: int) -> Dict[str, List[float]]:
...
def filter_weak_beats_in_intro(beats: List[float], y: np.ndarray, sr: int, 
                              intro_duration: float, 
                              strength_threshold: float = 0.3) -> List[float]:
    ...
    onset_times = librosa.frames_to_time(np.arange(len(onset_envelope)), sr=sr, hop_length=hop_length)
    ...
    for beat_time in beats:
        if beat_time > intro_duration:
            ...
            normalized_strength = beat_strength / max_strength if max_strength > 0 else 0
...
def analyze_audio(file_path: str) -> Dict[str, Union[float, List[float]]]:
    ...
    try:
        ...
        musical_start_time, intro_duration = detect_musical_start(y, sr, tempo, beat_frames)
