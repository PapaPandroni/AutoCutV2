...
def load_audio_with_ffmpeg_subprocess(audio_file: str) -> 'AudioArrayClip':
    ...
    logger.info(f"🔧 Loading audio with FFmpeg subprocess: {os.path.basename(audio_file)}")
    
    # FFmpeg command for reliable audio extraction
...
def load_audio_robust(audio_file: str) -> 'AudioArrayClip':
    ...
    logger.error(f"❌ {error_msg}")
    logger.error(f"   Audio info: {audio_info}")
