...
def load_audio_with_ffmpeg_subprocess(audio_file: str) -> "AudioArrayClip":
    ...
    try:
        ...
        error_msg = f"FFmpeg subprocess timed out after 60 seconds"
        logger.error(f"❌ {error_msg}")
        raise RuntimeError(error_msg)

    except Exception as e:
        error_msg = f"FFmpeg subprocess unexpected error: {str(e)}"
...
def load_audio_with_librosa(audio_file: str) -> "AudioArrayClip":
    ...
    try:
        ...
        error_msg = f"Librosa loading failed: {str(e)}"
        logger.error(f"❌ {error_msg}")
        raise RuntimeError(error_msg)
...
def _diagnose_ffmpeg_audioreader_error(error_str: str, audio_file: str, is_wav_file: bool) -> str:
    ...
    elif "object has no attribute 'proc'" in error_lower:
    ...
    elif "could not find codec parameters" in error_lower:
