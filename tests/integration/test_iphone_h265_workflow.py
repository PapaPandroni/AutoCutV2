...
class TestiPhoneH265Workflow:
    ...
    def test_complete_iphone_workflow(self, iphone_h265_file_path, temp_dir):
        ...
        assert basic_validation.is_valid, f"Basic validation failed: {basic_validation.errors}"
        ...
        iphone_validation = validator.validate_iphone_compatibility(str(iphone_h265_file_path))
        ...
        if codec_info.is_hevc:
            ...
            is_compatible = transcoding_service.test_moviepy_h265_compatibility(str(iphone_h265_file_path))
            ...
            if not is_compatible:
                ...
                if transcoding_result.success:
                    ...
                    pytest.fail(f"Transcoding failed: {transcoding_result.error_message}")
    ...
    def test_iphone_preprocessing_workflow(self, iphone_h265_file_path, temp_dir):
        ...
        processed_path = transcoding_service.preprocess_video_if_needed(str(iphone_h265_file_path))
        ...
        assert final_codec.codec in ['h264', 'hevc'], f"Unexpected codec: {final_codec.codec}"
    ...
    def test_hardware_specific_transcoding(self, iphone_h265_file_path, temp_dir):
        ...
        if capabilities.has_gpu_acceleration:
            ...
            assert result.encoder_used in ['nvenc', 'qsv'], f"Expected GPU encoder, got {result.encoder_used}"
        else:
            # CPU fallback
            assert result.encoder_used == 'cpu', f"Expected CPU encoder, got {result.encoder_used}"
    ...
    def test_validation_error_handling(self, temp_dir):
        ...
        iphone_validation = validator.validate_iphone_compatibility(str(fake_iphone_file))
        assert iphone_validation.validation_type == "iphone_compatibility"
    ...
    def test_performance_benchmarking(self, iphone_h265_file_path, temp_dir):
        ...
        if file_size_mb < 50:  # Small files should process quickly
            assert processing_time < 30, f"Small file took too long: {processing_time:.2f}s for {file_size_mb:.1f}MB"
        elif file_size_mb < 200:  # Medium files 
            assert processing_time < 120, f"Medium file took too long: {processing_time:.2f}s for {file_size_mb:.1f}MB"
        
        # Log performance for analysis
        print(f"Processed {file_size_mb:.1f}MB in {processing_time:.2f}s ({file_size_mb/processing_time:.1f} MB/s)")
...
class TestiPhoneH265ErrorScenarios:
    ...
    def test_transcoding_insufficient_disk_space(self, temp_dir, test_helpers):
        ...
        large_input = test_helpers.create_mock_video_file(temp_dir, "large_input.mov", 100.0)
        ...
        result = transcoding_service.transcode_h265_to_h264(str(large_input), str(output_path))
    ...
    def test_transcoding_permission_denied(self, temp_dir, test_helpers):
        ...
        transcoding_service = TranscodingService()
        result = transcoding_service.transcode_h265_to_h264(str(input_file), restricted_output)
    ...
    def test_ffmpeg_not_available(self, iphone_h265_file_path, temp_dir):
        ...
        with pytest.mock.patch('src.video.transcoding.subprocess.run') as mock_subprocess:
            mock_subprocess.side_effect = FileNotFoundError("ffmpeg: command not found")
            
            output_path = temp_dir / "output.mp4"
            result = transcoding_service.transcode_h265_to_h264(str(iphone_h265_file_path), str(output_path))
...
class TestiPhoneH265RealWorldScenarios:
    ...
    def test_multiple_iphone_files_batch_processing(self, sample_video_files, temp_dir):
        ...
        for video_file in iphone_files[:3]:  # Test first 3 files
            processed_path = transcoding_service.preprocess_video_if_needed(str(video_file))
            results.append({
        ...
        assert len(successful_results) == len(results), f"Some files failed to process: {results}"
    ...
    def test_iphone_transcoding_quality_verification(self, iphone_h265_file_path, temp_dir):
        ...
        if original_codec.is_hevc:
            ...
            if result.success:
                ...
                assert 0.5 < size_ratio < 3.0, f"Size ratio {size_ratio:.2f} outside acceptable range"
                
                # Resolution should be preserved
                if original_codec.resolution and transcoded_codec.resolution:
                    assert original_codec.resolution == transcoded_codec.resolution, "Resolution should be preserved"
                
                print(f"Transcoding quality check passed: {original_size_mb:.1f}MB → {transcoded_size_mb:.1f}MB (ratio: {size_ratio:.2f})")
            else:
