...
class TestFullPipeline:
    ...
    def test_complete_autocut_pipeline(self, sample_video_files, sample_audio_files, temp_dir):
        ...
        for video_file in video_files:
            ...
            assert result.is_valid, f"Video validation failed for {video_file}: {result.errors}"
        ...
        for video_file in video_files:
            preprocessed_path = transcoding_service.preprocess_video_if_needed(video_file)
            preprocessed_videos.append(preprocessed_path)
            assert Path(preprocessed_path).exists(), f"Preprocessed video not found: {preprocessed_path}"
        ...
        for video_file in preprocessed_videos:
            try:
                ...
                assert 'scenes' in analysis, f"Video analysis missing scenes for {video_file}"
                assert 'quality_score' in analysis, f"Video analysis missing quality score for {video_file}"
                assert len(analysis['scenes']) > 0, f"No scenes detected in {video_file}"
        ...
        try:
            ...
            assert Path(output_path).exists(), f"Output video not created: {output_path}"
            ...
            assert output_codec.codec is not None, "Could not detect codec of output file"
    ...
    def test_pipeline_with_hardware_acceleration(self, sample_video_files, sample_audio_files, temp_dir):
        ...
        print(f"Hardware-accelerated processing completed in {processing_time:.2f}s using {capabilities.best_encoder}")
    
    def test_pipeline_error_recovery(self, sample_video_files, sample_audio_files, temp_dir, test_helpers):
        ...
        good_videos = [str(f) for f in sample_video_files[:1]] if sample_video_files else []
        bad_video = str(test_helpers.create_mock_video_file(temp_dir, "corrupted.mp4", 0.1))
        ...
        try:
            ...
            pytest.fail(f"Pipeline crashed instead of handling error gracefully: {str(e)}")
    
    def test_pipeline_performance_benchmarks(self, sample_video_files, sample_audio_files, temp_dir):
        ...
        for num_videos, config_name in test_configurations:
            ...
            try:
                ...
                total_input_size_mb = sum(Path(vf).stat().st_size for vf in video_files) / (1024 * 1024)
                
                performance_results[config_name] = {
                    'num_videos': num_videos,
                    'processing_time': processing_time,
                    'input_size_mb': total_input_size_mb,
                    'throughput_mbps': total_input_size_mb / processing_time if processing_time > 0 else 0,
                    'success': Path(output_path).exists()
                }
                
                print(f"{config_name}: {processing_time:.2f}s for {total_input_size_mb:.1f}MB ({total_input_size_mb/processing_time:.1f} MB/s)")
        ...
        successful_configs = [k for k, v in performance_results.items() if v.get('success', False)]
        assert len(successful_configs) > 0, f"No configurations succeeded: {performance_results}"
    
    def test_pipeline_with_different_patterns(self, sample_video_files, sample_audio_files, temp_dir):
        ...
        successful_patterns = [k for k, v in pattern_results.items() if v.get('success', False)]
...
class TestPipelineComponentIntegration:
        ...
        for video_file in sample_video_files[:2]:
            ...
            if validation.is_valid:
                ...
                if codec_info.is_hevc:
                    iphone_validation = validator.validate_iphone_compatibility(str(video_file))
                    
                    # Step 4: Transcoding if needed
                    if not iphone_validation.is_valid or len(iphone_validation.errors) > 0:
                        output_path = temp_dir / f"integrated_transcode_{video_file.stem}.mp4"
                        ...
                        if transcoding_result.success:
                            ...
                            transcoded_validation = validator.validate_transcoding_output(str(output_path))
    ...
    def test_hardware_detection_to_transcoding_integration(self, sample_video_files, temp_dir):
        ...
        result = transcoding_service.transcode_h265_to_h264(str(test_file), str(output_path))
        
        # Verify the encoder used matches hardware capabilities
        if result.success:
            expected_encoder = capabilities.best_encoder
            assert result.encoder_used == expected_encoder, f"Expected {expected_encoder}, got {result.encoder_used}"
...
class TestPipelineStressTests:
    ...
    @pytest.mark.skipif(not pytest.config.getoption("--runslow", default=False), 
                        reason="Stress tests only run with --runslow")
    def test_pipeline_with_many_files(self, sample_video_files, sample_audio_files, temp_dir):
        ...
        try:
            ...
            if Path(output_path).exists():
                print(f"Stress test completed: {len(video_files)} files in {processing_time:.2f}s")
            else:
                print(f"Stress test failed after {processing_time:.2f}s")
            
            # Should complete within reasonable time (adjust as needed)
            assert processing_time < 300, f"Stress test took too long: {processing_time:.2f}s"
            
        except Exception as e:
            pytest.fail(f"Stress test failed: {str(e)}")
    
    def test_pipeline_memory_usage(self, sample_video_files, sample_audio_files, temp_dir):
